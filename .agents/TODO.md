...
- [x] Per-capability model selection (models use / config set / UI): vision catalog expanded to 4 vision pairs (bonsai-27b, gemma-3-4b-it, qwen2.5-vl-7b, qwen2.5-vl-3b) each bundling model+projector under one ID; `config.vision.local_model` (default `bonsai-27b`); `resolve_vision` uses selected catalog entry; `ghostreel-llm` builds prompts via model's chat template (fallback to ChatML) with Qwen-style `<think></think>` suppression; `models use <id>` (vision checked before generic whisper fallback); `config set vision.backend|url|model|stt.backend|url|embed.backend|url`; Tauri commands `get_ai_settings`, `set_ai_settings`, `server_models`, `probe_backends`; `ModelsPage` settings cards per capability (Auto/This computer/Server segmented control, URL + model dropdown + API key, probe status line, radio selection for local models, inline Download).
